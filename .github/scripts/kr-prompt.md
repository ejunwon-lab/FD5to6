...
- ✅ 허용: 지수·종목 등락·거래대금·수급(Step 1 fetch), 보유 리스트로 계산한 집중도, 업종 등락(Step 1 fetch).
...
### 1-F. 포트폴리오 지표 (익스포저%·MDD) — GAS, Bash 환경변수
...
```
→ JSON `{ assetClassWeights:{분류:%}, holdings:[{name,category,weight%}], mdd:음수% }`.
- 테마 익스포저: holdings의 weight%를 테마로 합산 — **반도체** = 삼성전자·SK하이닉스·삼성전기 + AI·반도체 ETF(반도체TOP10·로봇·소부장·AI반도체핵심장비 등).
- `mdd` = 고점 대비 최대 낙폭%(음수).
- env 미설정/실패/`success:false`면 정성 집중도로 degrade.
- ⚠️ **응답엔 원화 금액 없음(상대%·MDD만).** 리포트에 원화 절대액·총자산 금액 절대 쓰지 말 것(공개 리포트).
...
**2-C. 리스크**: 보유 16종의 **집중도**(반도체 익스포저% = 1-F holdings weight 합산 — 주식 삼성전자·SK하이닉스·삼성전기 + AI·반도체 ETF), **상관**(같이 움직였나=분산효과), **이상신호**(레버리지 ETF 과대 낙폭·공매도 등). 익스포저%·MDD는 **1-F 수치를 사용**(1-F 실패 시에만 정성 집중도로 degrade).

**2-D. 기회 파이프라인 갱신**: `_kr_pipeline.md` 상태 + 오늘 업종 강세 → ①최근 강세 섹터 갱신(연속 등장 = 추세 강도) ②후보 승급(신규→관찰→연구). **단발 급등(Top Movers 개별)은 노이즈로 제외, 섹터 추세 우선.** ③ 연구 후보(관찰 승급된 섹터)는 **구성종목에서 유동성 큰 대표주를 실제 선별**:
- WebFetch `https://finance.naver.com/sise/sise_group.naver?type=upjong`로 업종명→상세 링크(`no=`) 매핑 → `https://finance.naver.com/sise/sise_group_detail.naver?type=upjong&no={NO}`에서 **구성종목 표**(거래대금/시총 상위) 2~3 추출. (러너 fetch 정상 확인됨 — 단, 페이지 전체 링크가 아니라 *구성종목 표 행*만; 거래량 큰 것 우선)
- 못 뽑으면 지식 기반 대표주 `〔후보·미검증〕` 표기로 degrade. 등락 수치는 fetch 못 했으면 단정 금지.
...
• 익스포저 — 반도체 테마 {X}% (1-F holdings weight 합산), 자산군 {국내주식 A% / 해외ETF B% …}  _(1-F 성공 시. 실패면 정성 집중도만)_
• MDD — 고점 대비 {mdd}%  _(1-F. 자산액 기준·입출금 포함)_
