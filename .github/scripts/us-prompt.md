...
## Step 4. 휴장일 처리 — 직전 거래일 리뷰 (빈 메시지 금지)
미국 휴장이어도(Yahoo `regularMarketTime`이 전일) **"휴장" 한 줄로 끝내지 말고, 직전 거래일 마감 기준 PB 리포트를 정상 작성**한다:
- 헤더: `🌅 *US 마감 · 당신의 PB* · _직전거래일 YYYY-MM-DD(요일) 기준 · 휴장(사유)_`
- 시장 데이터: range=5d 종가 배열에서 **마지막 *실제 거래일* 종가 vs 그 직전 거래일**로 등락 계산(당일=휴장이라 등락 0이니, 마지막 거래일의 실제 등락을 본다). M7·섹터·AVGO 동일.
- 💼 내 포트(1-C GAS 지표는 휴장 무관 항상 최신) · 🧭 논리 · 🔮 전망 · 📅 이벤트는 **그대로 채운다**(직전 거래일 + 다가오는 거래일 기준).
- 다음 미국 거래일 명시.
> 빈 휴장 메시지 금지 — 사용자는 휴장에도 포트 점검·전망을 원한다. Step 2~3을 직전 거래일 기준으로 정상 수행.
