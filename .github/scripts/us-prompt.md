...
## ⛔ 절대 규칙 — 숫자·서사 출처 (환각 방지, 최우선)
- ✅ 허용: 1-A에서 fetch한 지수·M7·섹터·AVGO 등락. 1-B 기사에서 *직접 인용*한 Top Movers·이슈.
- ❌ 금지: 목표주가·컨센서스 수치(fetch 안 함), fetch 안 한 거시지표 단정. 추정은 `_(추정)_`.
- ❌ **거시·정책·지정학 서사**(연준 발언·도트플롯·FOMC 결과·CPI·지정학 딜·관세·정책 등): **1-B 기사에서 직접 읽은 것만**, 반드시 `(한경)`·`(인포맥스)` 식 **출처 표기**. 기사에 없으면 **언급 자체 금지** — 일반 지식·기억으로 메우지 말 것(이게 환각의 주 경로다). 구체 사건명·인물·수치를 출처 없이 단정하면 거짓이 된다.
- ✅ **해석·전망은 추론**: "~로 보인다", "~가능성" 등으로 *내 추론임을 명시*. 사실처럼 단정 금지. 가격·등락(검증된 사실)과 내 해석(추론)을 문장에서 구분.
> 틀린 숫자·거짓 거시 1개가 빠진 정보 10개보다 위험하다.
...
```
→ `{ assetClassWeights, holdings:[{name,category,weight%}], mdd, recentReturns, portfolioReturn:{d5,d20} }`. AVGO 비중 + 반도체 테마%(한국 반도체 + AVGO) + 최근 포트 수익률(`portfolioReturn.d5`=5일, `d20`=20일) 확인 → "미국 반도체 강세가 내 반도체 X%에 우호" 식 연결. **원화 절대액 절대 금지.** 비주식 항목명 인용 금지. env 없으면 이 부분 생략(실패시키지 말 것).
...
{1-C 있으면: 반도체 테마 {X}% · 최근 5일 {d5}% · MDD {mdd}%}
