...
- **🟡 텔레그램 푸시 F 구조 구현·배포 완료 — 6/10 실측만 남음 (2026-06-09)** — `/design-check` 첫 적용 → 프레임 전환(21번 외부 시계 → 1번 시작+자체 루프)으로 **F 구조 구현·배포·재배포 완료**. GAS `tgPushPnL`에 18분 dedup(`tg_lastPushEpoch`), `telegram-push.yml`을 sleep 루프(5분 poke)+4 cron redundancy로 재작성. push_safe 배포 후 **Apps Script API로 버전 고정 WEB_APP 배포 3개(v4/v5/v9)를 v10으로 직접 갱신**(HEAD는 이미 최신) → v10 콘텐츠에 dedup 실재 확인 → **스팸 위험 제거, 사용자 수동작업 0**. 사전 테스트 run 27202390183(메커니즘)·27204243708(프로덕션 스모크) ✅. 설계: `docs/plans/2026-06-09-텔레그램-스케줄러-신뢰성.md`. **🔴 남은 것 = 6/10(수) 09:00~16:00 KST 실측만**: 텔레그램 ~20분 간격 도착(≈22회) + `gh run list --workflow=telegram-push.yml`로 잡 시작 적중. 미달이면 redundancy 조정. (목표 21회 확정)
