...
- **🟡 텔레그램 푸시 daisy-chain 전환 완료 — 6/11(목) 라이브 관찰만 남음 (2026-06-10 밤)** — F 구조 spread가 6/10 오후 실측으로 반증됨(수정 후 잔여 18틱 중 **2틱만 발화** — "다음 틱 ~20분 내" 전제 붕괴, errors.md 6/10 밤 항목). → **daisy-chain 전환**: 각 run이 120분 루프 후 종료 직전 self-dispatch(`gh workflow run`, GITHUB_TOKEN의 workflow_dispatch 예외), `concurrency`로 증식 차단, cron은 재시드 백업 강등 → **정상상태 best-effort 의존 0회**. GAS v13: poke 응답 `result`(sent/skip-*) 에코(관측성) + getPortfolioMetrics 현금성 비중. **스모크 통과(2026-06-10 23:47)**: 시드 run 27284601406 → poke `result:"skip-offhours"` 정확 → "dispatch 성공(시도 1)" → 후속 run 27284616440이 **actor=github-actions[bot]로 자동 생성·실행** = 체인 가동 중. 설계: `docs/plans/2026-06-10-텔레그램-daisy-chain.md`. **6/11(목) 확인**: ① Telegram 09:00~09:05 첫 푸시 + ~20분 간격 ② run 로그의 result=sent 횟수(09:00~16:00 ≈ 21~22회) ③ 체인 연속성(2시간 간격 run 연쇄) — ②③은 Claude가 로그로 확인 가능. 실패 시에만 Cloudflare Worker cron 재논의.
  ...
  - 🟡 **market-report cron 지연 별건**: 6/9·6/10 KR 리포트가 +3.3~3.8h 지연 도착(~20:50 KST). 체인 1+ 거래일 검증 후 같은 메커니즘으로 정시화 검토 (errors.md "KR 시장 리포트" 항목).
