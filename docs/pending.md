...
- **🟡 데스크 iPad 가로모드 개선 트랙 (2026-08-06 시작)** — 완료: ①사이드바 4섹션 재편(+Activity 개명·Dividends 메뉴 제거·터치 타깃·힌트 pointer-fine) ②홀딩스 카드 무조건 접힘+독립 높이 ③Today 개편(KPI 5칸·기여도·주의 신호·분해·90일 위치·오늘 실현손익·2단 레이아웃) ④Today 뉴스(GAS News.js — 네이버 국내/해외)·이벤트 캘린더(정적+규칙). **잔여**: ⓐiPad 실기기 확인 ⓑ**GAS→네이버 실환경 스모크 = Today 뉴스 패널 첫 로드**(scripts.run 403·이 맥 .env 부재로 사전 실측 불가 — 패널이 실패/빈/성공 구분 표시) ⓒ**8-2 US 실적 캘린더 보류**(Yahoo quoteSummary crumb 락 실측 — 소스 재검토 필요) ⓓ매크로 캘린더 연도 갱신 규칙(2027 일정 웹 검증 후 macroCalendar.ts 추가). 세션: `docs/sessions/2026-08-06-데스크-iPad-사이드바재편.md`
