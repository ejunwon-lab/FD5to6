...
- **🟡 프론트 호스팅 GitHub Pages 의존 탈피 검토 (2026-07-10, 2026-08-06 재발)** — 7/9 Actions 광역 장애에 이어 **8/6 Pages 서빙 빌드 4연속 실패**(리포 단위, 콘텐츠 무관 — errors.md 8/6 항목). 데스크 브리지 UI 커밋(9a337b9)이 gh-pages까지 갔으나 미서빙, 사용자 '일단 대기' 결정. **다음 세션: Pages 빌드 상태 1회 확인(`gh api repos/ejunwon-lab/FD5to6/pages/builds/latest`) → 회복이면 종결, 여전히 실패면 Cloudflare Pages 이전 정식 착수**(재발 조건 충족). 대안 1순위 Cloudflare Pages(`wrangler pages deploy dist`), 비용 = base 경로 rebuild + OAuth 도메인 갱신.
- **🟡 데스크 로드맵 (2026-07-23 전수 분석 → 5단계, 사용자 승인)** — ①신뢰성 묶음 ✅ ②KIS Status 페이지 ✅(GAS newMobileGetSystemStatus) ③모바일 2단계 ✅(Today 컨트롤 가로스크롤 — Analysis·히트맵은 기대응 확인) ④포맷 로케일 통일 ✅(107곳 toLocaleString에 'ko-KR' 일괄 지정 — fmtKRW 전면 치환은 표기 회귀 위험 대비 이득 없어 제외) ⑤ 1차분 ✅(Price History·Settings·Phase C 3종[드로다운 underwater·분류 도넛·홀딩스 CSV]). **⑤ 잔여**: 실현손익 CSV, 상관관계 매트릭스(종목별 일봉 N회 조회 필요 — 비용 검토), 이코노믹 캘린더, Dividends(데이터 설계부터 — /design-check. 메뉴는 2026-08-06 제거 — 구현 시 Sidebar에 복원). ⑥Analysis 리스크 지표 자산 기반 교정 ✅(totalAsset API + Sharpe·Vol·MDD 분모 교체, TWR 한계는 주석 명시 — 완전 TWR 공용화는 후속 후보). 분석 상세: `docs/sessions/2026-07-23-수익로직감사-데스크반응형.md`
...
- **🟡 데스크 iPad 가로모드 개선 트랙 (2026-08-06 시작)** — 완료: ①사이드바 4섹션 재편(+Activity 개명·Dividends 메뉴 제거·터치 타깃·힌트 pointer-fine) ②홀딩스 카드 무조건 접힘+독립 높이 ③Today 개편(KPI 5칸·기여도·주의 신호·분해·90일 위치·오늘 실현손익·2단 레이아웃) ④Today 뉴스(GAS News.js — 네이버 국내/해외)·이벤트 캘린더(정적+규칙). **잔여**: ⓐiPad 실기기 확인 ⓑ**GAS→네이버 실환경 스모크 = Today 뉴스 패널 첫 로드**(scripts.run 403·이 맥 .env 부재로 사전 실측 불가 — 패널이 실패/빈/성공 구분 표시) ⓒ**GAS→Yahoo 실환경 확인 = 이벤트 패널에 US 종목 실적 행 유무**(crumb 2단 인증 로컬 실측 성공·GAS IP는 미검증 — 부재 시 Yahoo 차단 가능성, 뉴스와 독립 degrade) ⓓ매크로 캘린더 연도 갱신 규칙(2027 일정 웹 검증 후 macroCalendar.ts 추가). 세션: `docs/sessions/2026-08-06-데스크-iPad-사이드바재편.md`
