...
- **🟡 텔레그램 푸시 F 구조 구현·배포 완료 — 6/10 실측만 남음 (2026-06-09)** — `/design-check` 첫 적용 → 프레임 전환(21번 외부 시계 → 1번 시작+자체 루프)으로 **F 구조 구현·배포·재배포 완료**. GAS `tgPushPnL`에 18분 dedup(`tg_lastPushEpoch`), `telegram-push.yml`을 sleep 루프(5분 poke)+4 cron redundancy로 재작성. push_safe 배포 후 **Apps Script API로 버전 고정 WEB_APP 배포 3개(v4/v5/v9)를 v10으로 직접 갱신**(HEAD는 이미 최신) → v10 콘텐츠에 dedup 실재 확인 → **스팸 위험 제거, 사용자 수동작업 0**. 사전 테스트 run 27202390183(메커니즘)·27204243708(프로덕션 스모크) ✅. 설계: `docs/plans/2026-06-09-텔레그램-스케줄러-신뢰성.md`. **6/10 실측 결과 = 시작 드롭**: 오늘 telegram-push cron이 종일 미발화(틱 4개가 09:00·09:03 등 2시각에 몰려 GitHub 아침 윈도우 드롭에 전멸). 당일은 수동 dispatch로 복구(루프·dedup·발송은 라이브 정상이면 OK — 사용자 도착 확인 대기). **수정(커밋 7cfd479)**: cron `0,20,40 0-6 * * 1-5`(20분 간격 spread) + 루프 120분 → 한 윈도우 드롭돼도 다음 틱 ~20분 내 시작. **🟡 다음 = 6/11(목) 아침 모니터링**: 스케줄 잡이 09:00~09:40 KST 사이 시작하나 + 텔레그램 ~20분 간격. 또 드롭되면 **Cloudflare Worker cron**으로 전환(사용자 wrangler 인증 1회 필요 — 사용자 결정: spread 먼저). 설계: `docs/plans/2026-06-09-텔레그램-스케줄러-신뢰성.md`, 원인: errors.md 2026-06-10.
  - ✅ **손익 +0원 버그 수정·검증 완료(2026-06-10)**: 푸시 합계가 +0/직전값이던 건 `tgPushPnL`이 부분 갱신만 해 추이기록 AD2(=합계손익)·환율이 stale였던 것. `tgPushPnL`·`tgRefreshAndPush`를 `updateAllNew()` 전체 갱신으로 통일, v11 배포. ㄱㄱ 라이브 검증 OK(사용자 확인). 자동 푸시도 동일 코드라 정상. (errors.md 2026-06-10 "손익 +0원")

- **🟢 KR 리포트 B+C 개편 — Phase 1 완료 / Phase 2a 진행 중 (2026-06-10)** — 단순 리캡 → **의사결정 지원 + 기회 발굴**로 전면 개편(로테이션 렌즈 + 기회 파이프라인). **Phase 1 배포·검증 완료**: `kr-prompt.md` 재작성(⚡한줄·🔄자금흐름·🛡️수비·⚠️리스크·🎯파이프라인·💬다음수) + `kr-theses.md`(논리 초안 — **🔴 사용자 검토 필요**) + `_kr_pipeline.md`(다일 상태). 출처 가드레일로 환각 목표가 차단 확인(dry_run 2회). **Phase 2 진행**(설계: `docs/plans/2026-06-10-kr리포트-phase2.md`): 2b 섹터 구성종목 러너 진단 ✅통과(자동선별 실현가능), **2a 익스포저%·MDD 미구현** — GAS `getPortfolioMetrics`(상대%·MDD만, 원화0)+doPost → 배포 → market-report.yml KR step env로 GAS URL·시크릿 주입(public 프롬프트 미노출) → 프롬프트 연동 → dry_run(원화 미노출·환각0 게이트). MDD=추이기록 일별 Q열 peak-to-trough. **다음 세션 진입점.**
