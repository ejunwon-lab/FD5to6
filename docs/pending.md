...
- **🟡 데스크 로드맵 (2026-07-23 전수 분석 → 5단계, 사용자 승인)** — ①신뢰성 묶음 ✅ ②KIS Status 페이지 ✅(GAS newMobileGetSystemStatus) ③모바일 2단계 ✅(Today 컨트롤 가로스크롤 — Analysis·히트맵은 기대응 확인) ④포맷 헬퍼 통일(18파일 107곳 toLocaleString → fmtKRW) ⑤Price History → Settings → Phase C 선별(시장/섹터 도넛·드로다운 곡선·상관관계 등) → Dividends(데이터 설계부터). **⑥신규 발견: Analysis 리스크 지표 왜곡** — Sharpe·Volatility·Win Days·Best Day·MDD가 누적수익 곡선의 일별 %변화 기반(분모가 자산이 아니라 누적수익 → 과대). 올바른 계산엔 일별 총자산 시리즈(추이 기록 N~S, Q열) API 노출 필요 — /design-check 후 진행. Total Return은 summary 기준으로 즉시 교정 완료(2026-07-23). 분석 상세: `docs/sessions/2026-07-23-수익로직감사-데스크반응형.md`
