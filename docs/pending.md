...
- **🟡 텔레그램 푸시 F 구조 구현 완료 — 6/10 실측 대기 (2026-06-09)** — `/design-check` 첫 적용 → 프레임 전환(21번 외부 시계 → 1번 시작+자체 루프)으로 **F 구조 구현·배포 완료**. GAS `tgPushPnL`에 18분 dedup(`tg_lastPushEpoch`) 추가, `telegram-push.yml`을 sleep 루프(5분 poke)+4 cron redundancy로 재작성. 사전 테스트(run 27202390183) 메커니즘·연결 ✅. 설계·근거: `docs/plans/2026-06-09-텔레그램-스케줄러-신뢰성.md`. **🔴 블로킹 사용자 작업**: GAS 에디터 **배포 관리 → 신규 버전** (재배포 안 하면 옛 코드=dedup 없음 → 5분마다 스팸). **🔴 6/10(수) 09:00~16:00 KST 실측**: 텔레그램 ~20분 간격 도착 + `gh run list --workflow=telegram-push.yml`로 잡 시작 적중. 실측 미달이면 redundancy 조정. (목표치 21회 확정)
