...
- **🔴 리포트 private 분리 — 라이브 커밋 검증 (2026-07-04)** — 구조 전환·dry_run 통과(run 28702135651, private checkout 성공). **남은 것**: ① 다음 라이브 run(7/5 토 08:0x US·7/5 일 13:0x 주간)이 `FD5to6-reports`에 커밋을 남기고 main엔 커밋 없음 확인(`gh api repos/ejunwon-lab/FD5to6-reports/commits`) ② 🟡 분리 이전 리포트가 main 과거 히스토리에 공개 잔존 — 완전 제거(히스토리 재작성) 여부 사용자 결정.
