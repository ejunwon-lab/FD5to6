...
- **Today 뉴스 + 이벤트 캘린더 (8·9번)** — GAS `News.js` `newMobileGetNews` 신설(네이버 국내/해외 종목뉴스 무키 JSON — Yahoo RSS는 실측 사망으로 배제, 30분 CacheService, fetchAll 병렬, 종목 단위 degrade) + 데스크 NewsPanel(10건+더보기, 실패/빈 구분) + EventsPanel(FOMC·CPI·금통위 웹 검증 정적 일정 + 만기일 규칙 계산, D-day). /design-check 통과(docs/plans/2026-08-06-Today-뉴스-이벤트캘린더.md), push_safe 배포. US 실적 캘린더(8-2)는 Yahoo quoteSummary crumb 락 실측으로 보류. vitest 25/25.
- **US 실적 캘린더 (8-2, 보류 해제)** — Yahoo crumb 2단 인증(쿠키→getcrumb→quoteSummary) 로컬 실측 성공(MU→2026-09-23 추정)으로 보류 철회. GAS `_newsUsEarnings_` 추가(newMobileGetNews 응답 earnings[] 동승, 실패 시 빈 배열 degrade) + 데스크 useNews 공유 훅(패널 2곳 호출 1회 dedup) + EventsPanel 실적일 병합("(추정)" 라벨). push_safe 재배포, tsc·vitest 25/25·build 통과.
- **홀딩스 카드 가독성 2건 (사용자 피드백)** — ①헤더 숫자 3줄(현재가·당일 등락·당일 손익)에 흐린 소형 라벨 부착(접힘 2줄 헤더에도 현재가·오늘 라벨) ②매입/평가/수익 3열이 8자리 풀 숫자에서 겹치던 문제 → 매입·평가 윗줄 2칸 + 수익 아랫줄 중앙(굵게·색상 강조)으로 재배치. Web·Terminal 카드 동일 적용. tsc·vitest 25/25·build 통과.
- **홀딩스 카드 아코디언** — 기본(전체 접힘) 모드에서 카드를 펼치면 기존 펼침 카드가 자동으로 접히도록(foldOverrides 누적 → 단일 교체). '모두 펼치기' 모드는 개별 토글 유지.
- **Today 컨트롤 보강** — KPI 스트립에 KOSDAQ 추가 + 시장 타일 4개(KOSPI·KOSDAQ·S&P·환율)를 컴팩트 사이즈로(포트·Breadth 타일은 크게, lg 1.4fr:1fr 그리드), 종목 리스트에 대시보드와 동일한 계좌 필터 칩(전체/계좌별) 추가.
- **계좌 드릴다운 뷰** — Holdings > Account P&L의 주식 계좌 행 탭 → 계좌 전용 모달(KPI 6칸: 평가·투자원금·수익₩%·오늘 등락·실현손익 누적·대기자금 + 보유 종목 표 + 실현손익 최근 8건). 이미 로드된 데이터만 사용(추가 GAS 호출 없음), broker+계좌명 복합키 매칭.
- **Account P&L 상세 표 라디오 전환** — 주식/비주식/대기자금 카드의 상세 표가 순서대로 쌓이던 것 → 한 번에 하나만, 카드를 누르면 같은 자리에서 해당 표로 교체 (openSection 단일 상태).
- **Account P&L 탭 시각화** — 상세 표 닫힘 제거(항상 하나 열림, 기본 주식) + 선택 카드를 눌린 탭처럼(배경 밝게 + 하단 accent 보더, 주식/비주식 amber·대기자금 cyan) + 표 상단에 같은 accent의 섹션 타이틀 바("▾ 주식 계좌 상세 · N계좌 M종목") 신설 — 카드=탭, 표=탭 내용 구조 명시.
- **Account P&L 탭 바 신설 (UX 개선)** — 카드 아래·표 위에 명시적 탭 바("상세 | 주식 계좌 N | 비주식 자산 N | 대기자금 N", 선택=amber/cyan 채움 — 앱 필터 칩과 동일 언어) 부착. 카드=요약 숫자, 탭=상세 전환으로 역할 분리(카드 클릭도 여전히 탭 전환). 표 내부 SectionTitle 바는 탭 바로 대체·제거.
- **Account P&L 외곽선 연결 방식 (UX 재개선)** — 탭 바 제거, 선택 카드에 accent 외곽선 2px + 옅은 accent 배경 + 카드 아래 ▼ 연결 꼭지(lg+), 상세 표를 같은 accent 외곽선 박스(색 타이틀 스트립 포함)로 — 선택 카드와 상세가 같은 색 테두리 한 세트로 읽힘. 카드 그리드 gap-1.5·패딩으로 분리감, 순자산 카드는 amber/30 외곽선 정적 표시.
- **Account P&L 브리지 연결 (사용자 피드백)** — ▼ 꼭지 → 선택 카드 전체 폭 브리지로 교체: 카드 좌우 accent 보더가 그대로 내려가 카드 하단 보더·간격·상세 박스 상단 보더를 같은 배경으로 덮어 카드와 상세 박스가 한 덩어리(폴더 탭)로 보임. 카드·박스 보더 색 통일(amber/cyan full).
