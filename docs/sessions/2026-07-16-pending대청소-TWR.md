...
## 6. US 리포트 첫 시도 실패 수리 (저녁 추가)

- 실체: 리포트는 매일 도착(체인 재시도 성공) — 첫 dispatch만 9일간 5회 실패(⚠️ 알림·빨간 run이 사용자 체감). 실패 run 실측: 에이전트가 "파일 쓰기 권한 승인 필요" 산문으로 종료 + 파일명 US 마감일 오계산(US-07-14 vs 기대 07-15), 16분 소요(성공 9분 — 아침 Yahoo 429 턴 소모).
- 수리(3 job 일괄, `/design-check` 통과 — `docs/plans/2026-07-16-US리포트-첫시도실패-수리.md`): ①`--permission-mode bypassPermissions`(승인 대기 부류 제거, Bash 이미 무제한이라 보안 등가) ②max-turns 25→40 ③파일명 결정론화(워크플로가 DATE_KST 계산해 `[운영 지시]` 라인으로 프롬프트 주입 — 판정 step과 동일 식).
- 검증: YAML lint ✓, dry_run 스모크 dispatch(run 29490849045, 결과 대기) + 내일 08:0x 라이브가 최종 판정.
