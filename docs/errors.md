...
- **해결 (2026-07-23, 2단계)**: 1차 — 로컬 npm 11로 lock 재생성 → CI에서 **새 에러 EBADPLATFORM**(netbsd-arm64를 linux에 설치 시도 — npm 11이 쓴 optional 플래그를 npm 10이 오해석). 2차(확정) — **CI와 같은 메이저로 재생성**: `npx npm@10 install` → optional/os/cpu 플래그 정상, **npm 10·11 양쪽 `npm ci` 통과** + vitest 54·tsc·build 통과 → push로 배포 복구.
- **교훈**: ①package.json에 dep 추가하면 **같은 커밋에 lock 재생성** — npm ci 전환과 겹치면 배포가 조용히 죽는다. ②"로컬 npm ci 통과"는 CI 통과의 증거가 아님(npm 메이저별 lock 형식·검증이 다름) — **lock은 CI와 같은 npm 메이저(`npx npm@10 ...`)로 재생성**하고, 로컬 메이저로도 ci를 교차 확인. ③배포 워크플로 성공 여부는 watchdog 감시 대상이 아니었음 — push 후 deploy run conclusion 확인 습관 필요(이번에 백그라운드 대기로 잡음). ④부수 함정: 직전 명령이 `cd`를 남기면 다음 npm/rm이 엉뚱한 디렉토리에서 돈다 — npm 계열은 항상 절대경로 cd와 한 줄로.
...
- **증상**: 발현 기록 없음(잠복). 공개 ics 전수 분석 중 2026-06-03 '지방선거일'(DESCRIPTION='공휴일', KRX 휴장)이 `HOLIDAY_NAMES` 어느 단어와도 안 맞음을 확인 — 6/3 당일 *휴장일* 시트에 없었다면 제헌절(7/17)과 동일한 오발송·오기록이 있었을 것. **[확인 필요]** 주 맥북 백업 덤프의 *현재가_이력*에 2026-06-03 행 유무로 판정 가능(있으면 실제 발현).
- **원인**: 제헌절 건과 동일 클래스 — 이름 화이트리스트는 '선거' 계열 단어가 없어 선거일을 기념일 취급. 화이트리스트에 단어를 하나씩 추가하는 대응은 다음 미지 단어에서 또 뚫림.
- **해결 (2026-07-20)**: 채택 기준 자체를 교체 — 구글 캘린더 이벤트 **DESCRIPTION '공휴일' 분류**(ics 실측: 280건이 정확히 '공휴일' 205 / '기념일' 75로 이분, 제헌절·지방선거일·노동절 전부 '공휴일')를 1차 권위로. 화이트리스트는 desc 부재 시 폴백으로 강등(+'선거' 추가). `scheduledHolidaySync` 12월 게이트 제거 → 매월 25일 실행(연중 신설·재지정 ≤1개월 수용). push_safe 배포. 설계: `docs/plans/2026-07-20-휴장일-권위소스.md`.
- **교훈**: 손질 목록의 누락은 "터진 것만" 고치면 다음 누락이 남는다 — 목록 유지보수 자체를 없애는 권위 소스(제공자의 공식 분류)로 올라가야 클래스가 닫힌다. 외부 소스 검증은 배포 전에 공개 사본(ics)으로 실측 가능했다.

## 2026-07-17

### 제헌절(공휴일 재지정 첫날)에 텔레그램 푸시 발송 — 휴장일 화이트리스트가 법 개정으로 낡음
- **증상**: 2026-07-17 제헌절(KRX 전 시장 휴장)인데 장중 `tgPushPnL` 텔레그램 푸시가 계속 옴.
- **원인**: ① 제헌절이 2026-05-11 발효 법 개정으로 18년 만에 공휴일 재지정(첫 적용일이 당일) — *휴장일* 시트의 마지막 동기화(2025-12월 `scheduledHolidaySync`)는 발효 **이전**이라 2026-07-17 행 없음. ② 재동기화해도 `HOLIDAY_NAMES` 화이트리스트(스승의날 사고 2026-05-17의 해결책)에 '제헌절'이 없어 걸러짐. 화이트리스트의 구조적 약점: **세상(법)이 바뀌면 코드도 바뀌어야 함** — 연중 공휴일 신설은 12월 동기화 주기로도 못 잡음.
- **해결 (2026-07-17)**: Holidays.js 3곳 — `HOLIDAY_NAMES`에 '제헌절' 추가, KRX 고정휴장 루프에 `y>=2026 && 07-17` 고정 추가(캘린더 수록 여부와 무관하게 보장, 5/1·12/31 패턴), `_HOLIDAY_FALLBACK`에 2026-07-17 추가. push_safe 배포(HEAD — 메뉴/트리거 즉시 반영, 웹앱 버전 배포 불필요: 시트는 runtime 읽기). 응급조치: `gh workflow disable "Telegram Push (intraday P&L)"` + 실행 run 취소로 당일 푸시 차단, 16:05 백스톱으로 재가동+재시드. 시트 갱신은 메뉴 🗓️ 휴장일 동기화 1클릭(원격 함수 실행 경로 없음). 설계: `docs/plans/2026-07-17-제헌절-휴장일-반영.md`.
- **교훈**: 외부 세계의 목록(공휴일·수수료율 등)을 코드 화이트리스트로 박제하면 제도 변경 시 조용히 낡는다. 고정 날짜 규칙은 이름 매칭보다 코드 보장(5/1·12/31·7/17 패턴)이 견고. 잘못 기록된 당일 이력 행은 read-time `_isTradingDateStr` 필터가 응답에서 자동 드롭(자가치유, errors.md 2026-05-17 확장 건).
- **부수 발견**: push_safe 출력 `Protected on remote: none` — Secret.js는 원격에 더 이상 없음(시크릿은 PropertiesService로 이관 완료, Telegram.js 헤더). CLAUDE.md의 "Secret.js는 원격에만 존재" 서술은 낡음. push_safe는 원격 파일을 삭제하지 않는 merge 구조라 무해.

## 2026-07-16

### 추이기록 % 셀 타입 비대칭 — 음수 dRate가 100배 축소돼 d5 상방 왜곡
- **증상**: `getPortfolioMetrics` d5가 +8.04%로 나오는데, 일별 총자산 diff로 검산하면 -4.45%. 음수 날(7/13 실제 -7.08%)이 dailyReturns에 -0.07로 나옴.
- **원인**: `_trFmtPct`(Trend.js)가 `'+5.04%'`/`'-7.08%'` **문자열**을 setValues로 쓰는데, Sheets가 **음수 % 문자열은 분수 numeric으로 자동 파싱**(-0.0708)하고 **`+` 접두 문자열은 텍스트로 보존**함. 읽기 쪽 `parseFloat(String(v).replace(...))`는 numeric 셀을 -0.0708 → "-0.07%"로 해석 = **음수 날만 100배 축소**. S(일별 dRate)·AC(운용수익률)·AF(합계 변동률) 전부 동일. 검산: 7/13 Q열 diff -7.0815% vs S셀 -0.0708 numeric — 분수 해석이 정확 일치.
- **해결 (v30, 2026-07-16)**: 읽기 쪽 `_mPctVal(v)` — `typeof v === 'number'`면 ×100, 텍스트면 parseFloat. dailyReturns(S)·recentReturns(AC)·`_mFindPrevDayProfitChange`(AF) 3곳 적용. **근본 수정(v31, 같은 날)**: 쓰기 쪽 `_trFmtPct`를 numeric 분수 반환 + `_trPctFormat` 숫자서식(`'+0.00%;-0.00%;+0.00%'`)으로 전환 — 신규 행은 타입 통일, 표시 동일. 과거 혼합 행 때문에 읽기 파서는 유지. 설계: `docs/plans/2026-07-16-Trend-pct-numeric.md`. 구세대(2025년~2026-05-13) 행은 fraction/percent 혼재로 모호하나 소비 창(최근 5~20거래일)에 안 닿음. POST 실측: d5 +8.04→**-4.45%**, 7/13 -0.07→**-7.08** 교정.
- **교훈**: **부호 접두 % 문자열을 setValues로 쓰면 Sheets가 부호에 따라 타입을 갈라 저장**한다(`+` = 텍스트, `-` = 분수 numeric). % 컬럼은 numeric+숫자서식으로 쓰거나, 읽기에서 typeof 분기 필수. "같은 컬럼인데 양수 날만 맞는" 증상이면 이 함정부터 의심.

### 주간 수익률 입금 왜곡 (TWR) — 입출금이 수익으로 오인 [기능 부재→보정 구현]
- **증상**: 6/28 주간 리포트 d5 +4.9% vs 실질 약 -6% (6/23 대규모 입금). 7월 둘째 주엔 예금 만기·계좌 정리로 총자산 -4~-7% 급변일 4건(7/8·9·13·16)이 손실처럼 잡힘.
- **원인**: dRate = 총자산 diff — 입출금·계좌간 이동을 시장 수익과 구분 못 함. 입출금이 일자별로 기록되는 곳 없었음(원장은 매수/매도만, 대기자금은 상태 덮어쓰기).
- **해결 (v29~v30, 2026-07-16)**: ① *거래_원장* `구분=입금/출금` 행 타입(이력 append, `NS.FLOW_TYPES`) + addTrade POST 확장(멱등: 날짜+구분+증권사+계좌+금액) ② `getPortfolioMetrics`가 read-time TWR 보정: `V_prev=Q/(1+S)`, `r_adj=(Q−F−V_prev)/V_prev` → `flowAdj:true`+`rawPct` 동반, 소급 기록 시 자동 자가치유 ③ |dRate|≥3%인데 flow 기록 없으면 `suspect:true`. 설계: `docs/plans/2026-07-16-TWR-입금왜곡보정.md`.
- **교훈**: 이벤트성 데이터(입출금)는 처음부터 append 이력으로 — 상태(대기자금 셀) 덮어쓰기는 과거를 지워 소급 보정 불가. read-time 보정으로 설계하면 과거 gap도 소급 기록으로 치유 가능.

## 2026-07-15

### KIS_SKIP(예금) 행 운용수익 오염 — 평가금액 빈칸 + 손익 셀에 전체집계 수식
- **증상**: *보유현황* 우리은행 정기예금 5년 행의 손익(운용수익)이 **[금액]**(매입 [금액]의 17배). 대시보드·desk 비주식자산도 동일 오염. 펀드·보험 행은 정상.
- **원인**: 예금은 `NS.KIS_SKIP`이라 `updatePositionFromLedger`가 시세·손익을 계산하지 않고 사용자 입력을 **백업→복원(보존)만** 함(NewSystem.js:685-694·720-730). 사용자가 **현재단가([금액])는 넣었으나 평가금액(K열)을 비워둠** + 손익(L열)에 전체 운용수익을 가리키는 잘못된 수식이 남음. 코드가 이 셀을 안 고쳐 영구 지속. **검산**: opProfit=Σ평가−Σ매입에서 예금 평가=0·매입 포함 → [금액]−[금액]=**[금액]**(=오염 손익과 정확 일치)로 원인 확정. 정상값은 *분석* 시트가 현재단가에서 자체 계산해 +[금액](13.49%)로 표시 중이었음.
- **해결 (v28, 2026-07-15)**: KIS_SKIP 행 처리에 **자가치유** — 평가금액 비었고 현재단가>0이면 **평가=현재단가×수량, 손익=평가−매입, 수익률=손익/매입 재계산**(`skipAutoFilled`), 그 행은 수식복원에서 평가/손익/수익률(idx 10·11·12) **제외**(잘못된 집계 수식 제거). 평가금액이 이미 채워진 펀드·보험은 분기 안 타 **완전 불변**. 운용수익 [금액]→**[금액]** 교정. 설계: `docs/plans/2026-07-15-KIS_SKIP-운용수익-자가치유.md`.
- **교훈**: "코드가 계산 안 하고 보존만 하는 셀"(KIS_SKIP 수동입력)은 한 번 오염되면 자가교정 안 됨 → 최소한 **불변식(손익=평가−매입)을 코드가 강제**하고, 빠진 입력은 다른 입력(현재단가)에서 유도. 소비처(summary opProfit·desk 비주식자산)가 같은 셀을 읽으므로 교정이 동시 전파.
...
### 거래_원장에 직접 타이핑한 거래는 재계산이 자동으로 안 걸림 (2026-07-09)
- **증상**: 거래_원장 시트에 매도 행을 수기로 넣었는데 보유현황·실현손익에 반영 안 됨 (예금 만기 정산 시)
