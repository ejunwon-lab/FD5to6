...
- `components/shell/` — `TopBar`, `Ticker`(live indicators + holdings movers), `Sidebar`(lg+ 고정, 미만 드로어), `MobileTabBar`(모바일 하단 탭 6개 — Workspace 전환, Data/System은 드로어), `Footer`(lg+만)
- `components/dashboard/` — `DashboardPage`, `KpiStrip`, `MarketIndices`, `EquityChart`(범위 버튼은 달력일 날짜 산술로 필터 — slice 아님), `ActivityFeed`, `DashboardHoldings`, `HoldingsStatusStrip`, `HoldingsTable`, `Indicators` · `DashboardHoldings`는 기본 상위 9종목 접기(전체 보기 토글, 합계·strip은 전체 기준) + 카드 폴드 2단(접힘=헤더만↔펼침=요약+상세, 기본: 데스크톱 펼침/모바일 접힘, '모두 접기' 전역 토글, foldOverrides로 개별 오버라이드)
