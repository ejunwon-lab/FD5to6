...
- `onOpen` — **📊 뉴시스템**(데이터 작업) + **🛠️ 유지보수**(진단·트리거) 두 메뉴 생성
  ...
  - 유지보수: 🔍 진단(`runDiag`) / ⏰ 17:30 트리거 / 🕐 장중 매시 :30 트리거
...
### Telegram.js — Telegram 봇 연동 (워치/모바일 손익 알림)
...
- `tgPushPnL` — 자동 푸시 (트리거에서 호출). **거래일 + 09:00~16:00**만 실제 발송, 그 외 생략. 갱신 없이 시트 현재값으로 푸시
- `tgRefreshAndPush` — 수동 갱신 (가격 + 보유현황 재계산 후 푸시). 대시보드·추이 렌더 생략 (응답 시간 단축)
- `doPost`/`handleTelegramWebhook(e)` — Telegram webhook 진입점 (Main.js의 doPost가 위임)
...
- Setup 함수 (에디터에서 직접 실행):
  ...
  - `tgSetupPushTrigger` / `tgDeletePushTrigger` — 매시 :00/:20/:40 근처 트리거 3개 (GAS는 `everyMinutes(20)` 미지원이라 `everyHours(1).nearMinute(N)` 3개로 분리)
...
- `components/holdings/` — `HoldingsPage`(Account P&L + DashboardHoldings 재사용), `ExposureMatrix`(Account P&L 패널, 주식 계좌별 → 비주식 자산 → 대기자금 → 순자산 합계), `HoldingCard`(Terminal), `HoldingCardWeb`(Web 스타일), `StockDetailModal`
...
- `components/indicators/` — `IndicatorsPage`(Gainers/Losers strip + Market Heatmap + 카테고리 패널), `GainersLosersStrip`, `MarketHeatmap`
- `components/analysis/` — `AnalysisPage`(KPI·Allocation·Concentration·Contribution·Winners/Losers), `ContributionBar`(종목별 합계 수익 기여, ₩/% 토글)
- `components/activity/` — `ActivityPage`(KPI 6칸 [Realized P&L·Win/Loss·Win Rate·Avg Trade·**Total Fees**] + Monthly 막대 [풀 숫자] + 9컬럼 테이블 [Date·종목(이름 메인+코드 보조)·증권사·계좌·수량·매수가·매도가·수수료·손익·수익률] — 매도일 desc)
