...
- `doPost(e)` — `action=addMarketReport`면 `_tgHandleMarketReportPost`(Telegram.js)로 위임, 그 외는 `handleTelegramWebhook`
...
- **시장 리포트 큐** (Claude Routines → 시트 → Telegram, `docs/market-report-routines.md` 참조):
