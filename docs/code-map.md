...
| 당일 수익 금액·% (앱 대시보드) | GAS `MobileAPI.js` | `_mCalcExtras`(change) · `newMobileGetPortfolio` |
| 합계/운용/확정 수익 | GAS `MobileAPI.js` + *추이 기록*(AD2) | `newMobileGetPortfolio` · `_mFindPrevDayProfitChange` |
| 1주/1달 손익 (*대시보드* 시트) | GAS `Dashboard.js` | `_calcExtraColumns` · `pnlAt` |
| 1M/3M/6M/1Y 수익률 | GAS `MobileAPI.js`(앱) · `Dashboard.js`(시트) | `_mCalcExtras`/`_calcExtraColumns` 의 `pctAt` |
| 52주 고저 (high52/low52) | GAS `MobileAPI.js` | `_mCalcExtras` |
...
| 거래일/휴일 판정 | GAS `MobileAPI.js` | `_isKoreanHoliday` · `_mIsTradingDay` · `_isTradingDateStr` |
