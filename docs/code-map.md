...
- `components/shell/` — `TopBar`(DATA LIVE/SAMPLE·KRX/NYSE 개장 실배선 — 요일·시각 근사, 공휴일 미반영), `Ticker`(live indicators + holdings movers), `Sidebar`(lg+ 고정, 미만 드로어), `MobileTabBar`(모바일 하단 탭 6개 — Workspace 전환, Data/System은 드로어), `Footer`(lg+만)
- `components/dashboard/` — `DashboardPage`, `KpiStrip`, `MarketIndices`, `EquityChart`(범위 버튼은 달력일 날짜 산술로 필터 — slice 아님), `ActivityFeed`(실현손익 최근 8건 배선, 빈 데이터만 SAMPLE 폴백), `DashboardHoldings`, `HoldingsStatusStrip`, `Indicators` · `DashboardHoldings`는 Settings 기반 상위 N 접기·뷰모드·폴드 기본값 + ⬇CSV(필터 상태 그대로, `lib/csv.ts` BOM 포함) + 카드 폴드 2단(접힘=헤더만↔펼침=요약+상세, 기본: 데스크톱 펼침/모바일 접힘, '모두 접기' 전역 토글, foldOverrides로 개별 오버라이드)
...
- `components/system/SettingsPage.tsx` — Settings(단축키 S): localStorage `desk_settings_v1`(`lib/settings.ts` — theme[modern/terminal]·holdingsViewMode·holdingsInitialVisible[0=전체]·cardFoldDefault[auto/folded/unfolded]) — DashboardHoldings가 마운트 시 소비, theme는 `applyTheme()`가 `<html data-theme>` 스탬프로 즉시 반영
