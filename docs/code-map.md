...
- `components/analysis/` — `AnalysisPage`(KPI·**BenchmarkPanel**·Allocation·Concentration·Contribution·Winners/Losers), `ContributionBar`(종목별 합계 수익 기여 ₩/% 토글), `BenchmarkPanel`(Portfolio vs KOSPI·S&P recharts LineChart — IndicatorHistory lazy load, 시작점 0% 정규화, vs KOSPI·vs SPX outperformance KPI 카드)
