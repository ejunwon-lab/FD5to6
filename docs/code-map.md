...
- `getPortfolioMetrics` / `_handlePortfolioMetricsPost` — **포트폴리오 상대 지표** (리포트 public용, doPost `action=portfolioMetrics`+시크릿). 자산군 비중%(현금성 포함)·종목 비중%·MDD%·`recentReturns`(운용수익률% 누적 시계열, AC열)·`dailyReturns:[{date,dRatePct}]`(최근 7거래일 **일별 총자산 변화율%**, S열 — 마지막5 복리=d5라 주간 리포트 일자별 표와 d5 정합)·`portfolioReturn{d5,d20}`(일별 자산변화율 dRate 복리누적, 매매 강건·벤치 단위일치) — **원화 절대액 미포함**. MDD=*추이 기록* 일별 Q열 peak-to-trough, dailyReturns·portfolioReturn=일별추이 S열(dRate)
- `_handleEmailReportPost` / `_emailReportSelfTest` / `emailKillSwitch_ON/OFF` / `_mdToHtml` — **이메일 셀프발송** (PB 리포트, doPost `action=emailReport`+시크릿). 수신=발송=`getEffectiveUser` 소유자 고정(제3자 불가). 안전장치 5중: killswitch(`email_disabled`)·제목화이트리스트(`Market Close/Wrap`)·일일상한 6·주소마스킹·수신자고정. `_mdToHtml`=MarkdownV1→HTML(htmlBody). 스코프 `script.send_mail` 필요(재승인)
...
- `us-prompt.md` — US 분석 prompt (Yahoo WebFetch 정본 + 인포맥스 RSS·한경 글로벌마켓 보조, WebFetch 실패 시 curl 폴백)
- `kr-prompt.md` — KR 분석 prompt (Naver·뉴스는 **Bash curl 필수** — WebFetch 도구 계층 차단, 2026-07-03. finance.naver.com은 iconv euc-kr. 수급/업종/종목뉴스API/Top Movers + 보유종목 그룹별)
- `weekly-prompt.md` — 주간 리캡 prompt (GAS 지표는 워크플로 pre-fetch 파일 `/tmp/weekly/gas_metrics.json` Read 1차, env curl 폴백. Yahoo curl 429 재시도)
