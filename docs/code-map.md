...
- `_mGetBuyDates` / `_mGetFxRates` / `_mGetCashReserve` (*설정* A7:E12) / `_mFindPrevDayProfitChange`
...
- `components/holdings/` — `HoldingsPage`(Account P&L + DashboardHoldings 재사용), `ExposureMatrix`(Account P&L 패널, 계좌×(원금/수익금/평가)), `HoldingCard`(Terminal), `HoldingCardWeb`(Web 스타일), `StockDetailModal`
...
- `components/activity/` — `ActivityPage`(월별 P&L)
