...
| 기간별 수익 (1주/1개월/올해) | web 순수함수 (*추이 기록* AD diff, 서버 윈도 계약: 최소 180거래일+전년 12/1) | `computePeriodProfits` (web only) |
...
- `_newsUsEarnings_` — US 보유 종목 다음 실적일. Yahoo quoteSummary calendarEvents (fc.yahoo.com 쿠키→getcrumb→crumb 파라미터 2단 인증). 응답 `earnings[]`{code·name·date·estimate}로 뉴스에 동승, 실패 시 빈 배열
...
- `utils/periodProfit.ts` — `computePeriodProfits`(1주/1개월/올해 수익 = *추이 기록* AD diff, 실현+평가 포함. YTD 정확성은 서버 윈도 계약[최소 180거래일+전년 12/1]에 의존). vitest 6케이스
- `components/dashboard/` — `DashboardPage`(합계/오늘수익/기간별수익/자산배분/계좌유형별/환율 — 매도복기는 전용 탭으로 분리), `ProfitHistoryChart`
...
- `components/today/` — `TodayPage`(단축키 Y, Portfolio > Today — 2026-08-06 개편: KPI 6칸 [포트 변동·Breadth 큰 타일 + KOSPI·KOSDAQ·S&P500·USD/KRW 컴팩트 타일] + 오늘 실현손익 조건부 스트립 [realized entries 오늘 날짜 필터] + lg 2단 [좌 종목 리스트(계좌 칩 필터·Sort·Filter) / 우 인사이트 스택, 모바일은 인사이트 먼저]), `ContributionPanel`(오늘 ₩변동 플러스/마이너스 기여 Top5 가로 막대 — 계좌 합산), `SignalsPanel`(주의 신호 — 수익↔손실 전환·±3% 급등락·52주 고/저점 근접), `BreakdownPanel`(시장 KR/US·계좌별 오늘 ₩분해 + 오늘 등락의 최근 90거래일 분포 내 위치), `NewsPanel`(보유 종목 뉴스 10건+더보기, 실패/빈 상태 구분 표시 — GAS→네이버 실환경 스모크 신호), `EventsPanel`(macroCalendar 정적 이벤트 + useNews earnings 실적일 병합 8건 D-day)
- `lib/useNews.ts` — 뉴스+실적일 공유 훅 (gasApi.getNews 모듈 캐시 30분 + inflight dedup — NewsPanel·EventsPanel 동시 마운트에도 호출 1회)
...
- `components/holdings/` — `HoldingsPage`(ExposureMatrix + **Position52WeekPanel·ReturnHistogramPanel** 2단 + **AccountTypePanel** + DashboardHoldings 재사용), `ExposureMatrix`(Account P&L 패널 — 주식 계좌 행 탭 → `AccountDetailModal` 계좌 드릴다운 [2026-08-06]: KPI 6칸(평가·원금·수익·오늘·실현누적·대기자금) + 보유 종목 표 + 실현손익 최근 8건, 로드된 holdings·realized·cashReserve만 사용해 추가 GAS 호출 없음), `AccountTypePanel`(계좌 유형별 일반투자/퇴직연금·증권사 계좌 분해 — `accountType.ts` `computeAccountTypeBreakdown` 클라 계산, 대기중 포함=총자산 기준), `Position52WeekPanel`(top 12 by weight·종목별 가로 막대 [저점─●현재가─고점]·25%/75% 색 임계), `ReturnHistogramPanel`(10구간 막대 분포·median/mean 메타·gain/loss 색상 구분), `HoldingCard`(Terminal)·`HoldingCardWeb`(Web 스타일) — 2단 폴드: isExpanded=false면 컴팩트 2줄(종목명|현재가 / 등락%·등락총액)만, true면 요약+상세 9필드(중복 3필드 제거). 2026-08-06: 헤더 숫자 3줄에 라벨(현재가·{changeLabel} 등락·{changeLabel} 손익) 부착, 요약은 매입·평가 윗줄 2칸+수익 아랫줄 중앙(3열 풀 숫자 겹침 해소), `StockDetailModal`
...
- `components/analysis/` — `AnalysisPage`(KPI[리스크 지표는 totalAsset 자산 시리즈 기반 — 자산 없으면 수익곡선 폴백, Total Return은 summary 우선] + **DrawdownPanel**(underwater 자산 peak 대비 % AreaChart) + **분류 도넛**(category별 집계)·**BenchmarkPanel**·**MonthlyHeatmapPanel**·Allocation·Concentration·Contribution·Winners/Losers), `ContributionBar`, `BenchmarkPanel`(Portfolio vs KOSPI·S&P recharts LineChart — IndicatorHistory lazy load, 시작점 0% 정규화, vs KOSPI·vs SPX outperformance KPI 카드), `MonthlyHeatmapPanel`(equityCurve 월말 - 전월말 변화 격자, 윈도 첫 달은 baseline 전용으로 제외[누적 부풀림 방지], 색 진하기=|Δ|/max, hover 풀 ₩, YTD 합)
...
- `components/pricehist/PriceHistoryPage.tsx` — Price History(단축키 P): 종목 리스트(코드 dedup·평가금액순, 모바일 가로칩) → `newMobileGetStockDetail` priceHistory 차트(1M/3M/6M/ALL 날짜 산술·매수/매도 마커·내 평균단가 ReferenceLine·기간 KPI)
- `components/system/SettingsPage.tsx` — Settings(단축키 S): localStorage `desk_settings_v1`(`lib/settings.ts` — theme[modern/terminal]·holdingsViewMode·holdingsInitialVisible[0=전체]. cardFoldDefault는 2026-08-06 제거 — 항상 접힘 시작) — DashboardHoldings가 마운트 시 소비, theme는 `applyTheme()`가 `<html data-theme>` 스탬프로 즉시 반영
- **테마 시스템** — 모든 색은 `src/index.css`의 RGB 채널 변수(`--c-*`)로 정의, `tailwind.config.js`가 `rgb(var()/alpha)`로 소비. `:root`=terminal(네온 원형)·`[data-theme='modern']`=가독성 개선판(Pretendard·소프트 팔레트·대비≥4.5:1). 차트 시리즈는 별도 `--c-s1~s7`(modern은 CVD 검증 팔레트). 폰트 크기 `--fs-*`는 모바일(<640px)에서 한 단계 상향. 컴포넌트/차트에 raw hex 금지 — `rgb(var(--c-…))` 사용
- `components/system/KisStatusPage.tsx` — KIS Status(단축키 K): `newMobileGetSystemStatus` — KIS 시세 carried/total·시스템 상태·지표 충족도 바·최근 이력/휴장일. 금액 없는 진단 페이로드만
