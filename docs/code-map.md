...
- `getPortfolioMetrics` / `_handlePortfolioMetricsPost` — **포트폴리오 상대 지표** (리포트 public용, doPost `action=portfolioMetrics`+시크릿). 자산군 비중%(현금성 포함)·종목 비중%·MDD%·`recentReturns`(운용수익률% 누적 시계열)·`portfolioReturn{d5,d20}`(일별 자산변화율 dRate 복리누적, 매매 강건·벤치 단위일치) — **원화 절대액 미포함**. MDD=*추이 기록* 일별 Q열 peak-to-trough, portfolioReturn=일별추이 S열(dRate) 복리누적
