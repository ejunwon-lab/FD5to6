...
- `updatePositionFromLedger` — *거래_원장* → *보유현황* 재계산. 끝에 `computeStockMetrics`·`buildSoldTracker`·`buildDashboard` 호출. KIS_SKIP(예금·펀드·보험) 행은 값·수식 보존, **단 평가금액 빈칸+현재단가>0이면 자가치유**(평가=현재단가×수량·손익=평가−매입 재계산·잘못된 집계 수식 제거, errors.md 2026-07-15)
...
- `components/dashboard/` — `DashboardPage`(합계/오늘수익/기간별번돈/자산배분/매도복기/환율), `ProfitHistoryChart`, `SoldTrackerCard`(매도 복기)
